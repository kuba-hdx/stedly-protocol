/**
 * GradientBlinds — ported from reactbits.dev (React Bits)
 * Uses ogl (loaded from CDN as global `ogl`).
 *
 * Props: gradientColors, angle, noise, blindCount, blindMinWidth,
 *        mouseDampening, mirrorGradient, spotlightRadius, spotlightSoftness,
 *        spotlightOpacity, distortAmount, shineDirection, mixBlendMode,
 *        paused, dpr, className, style
 */
...
function GradientBlinds({
  ...
  React.useEffect(() => {
    ...
    const start = () => {
      if (cancelled) return;
      if (!window.ogl) {
        // wait for ogl-ready event or poll briefly
        const onReady = () => { window.removeEventListener("ogl-ready", onReady); start(); };
...
void main() {
  ...
  gl_FragColor = color;
    ...
    // Track pointer on the whole window so cursor effect is felt across the hero area
    ...
    dpr, paused, JSON.stringify(gradientColors), angle, noise, blindCount, blindMinWidth,
    mouseDampening, mirrorGradient, spotlightRadius, spotlightSoftness, spotlightOpacity,
  ...
  return (
    <div
      ...
      style={{ ...(mixBlendMode && { mixBlendMode }), ...(style || {}) }}
    />
  );
}
