/* ColorBends — Three.js shader background component
   Adapted from React Bits for our non-module Babel setup.
   Depends on window.THREE (loaded via CDN in Stedly Landing Page.html). */
...
`;
  ...
  function ColorBends({
    ...
    useEffect(() => {
      if (!THREE) {
        console.error("[ColorBends] THREE is not loaded");
        return;
      }
    ...
    useEffect(() => {
      ...
      JSON.stringify(colors), transparent
    ]);
    ...
    useEffect(() => {
      ...
      const handlePointerMove = (e) => {
        ...
        if (pointerTargetRef.current) pointerTargetRef.current.set(x, y);
      };
